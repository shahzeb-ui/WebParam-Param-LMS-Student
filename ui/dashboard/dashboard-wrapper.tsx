...
const InstructorDashboardHeader = () => {
  ...
  const flags = useFlags(["COURSE_ID", "DEMO", "BANNER_URL"]);
  const isBannerUrl = flags.BANNER_URL.enabled && flags.BANNER_URL.value;
  ...
  async function getEnrollment(userId: string) {
    try {
      const res = await axios.get(
        `${rCourseUrl}/api/v1/Enrollments/GetUserEnrollment/${userId}`
      );
  ...
  async function getCourse(courseId: string) {
    const res = await axios.get(
      `${rCourseUrl}/api/v1/Courses/GetCourseNew/${courseId}`
    );
  ...
  useEffect(() => {
    if (flags.COURSE_ID.enabled && flags.COURSE_ID.value) {
      getCourse(`${flags.COURSE_ID.value}`);
    }
  }, []);
  ...
  return (
    <>
      <div className="mb-5">
        {flags.DEMO.enabled && flags.DEMO.value == true ? (
      ...
      <div className="rbt-dashboard-content-wrapper">
        {isMobile && (
          <div
            ...
            style={{
              backgroundImage: `url(${
                isBannerUrl ? flags.BANNER_URL.value : ""
              })`,
