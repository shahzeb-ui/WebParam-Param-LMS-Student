...
const CourseWidgets: React.FC<Props> = ({
    ...
    useEffect(() => {
        const calculateDiscount = () => {
            const discount = ((data.coursePrice - data.offerPrice) / data.coursePrice) * 100;
            setDiscountPercentage(discount.toFixed(0));
        };

        const calculateTotalReviews = () => {
            const total = data.reviews.oneStar + data.reviews.twoStar + data.reviews.threeStar + data.reviews.fourStar + data.reviews.fiveStar;
