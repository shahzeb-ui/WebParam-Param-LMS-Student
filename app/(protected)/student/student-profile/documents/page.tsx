...
const FileUpload: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    if (selectedFile && selectedFile.file) {
      ...
      const type = documentsRequired.findIndex(
      ...
      try {
        const response = await axios.post(
          `${wUserUrl}/Profile/SubmitDocument`,
          formData,
          {
            headers: {
    ...
    if (selectedFile && selectedFile.file) {
      ...
      try {
        const response = await axios.put(
          `${wUserUrl}/Documents/UpdateStudentDocument`,
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
  ...
  return (
    <>
          ...
          <div className="modalContainer">
            {upLoadingLoader ? (
              ...
              <>
                ...
                <p>{selectedFile?.file?.name}</p>
                <button onClick={() => handleChangeDocument(docToChange)}>
                  Upload
                </button>
              </>
            )}
      ...
      >
        ...
        <Modal.Body>
          <Worker workerUrl={pdfWorkerUrl}>
            <Viewer
              fileUrl={`${rUserUrl}/Documents/PreviewDocument/${documentToView}`}
      ...
      >
        <span className="fw-semibold">Attention!</span> Please upload all your
      ...
      <div className="requiredDocs">
        {documentsRequired.map((doc, index) => {
          ...
          return (
            <div
              className={`docContainer ${matchingDoc && "uploaded"} ${
                dragging[docType] ? "dragover" : ""
              }`}
              key={index}
            ...
            >
              ...
              <div>
                {matchingDoc ? (
                  <i
                    ...
                    style={{ cursor: "pointer", color: "#dfedfa" }}
              ...
              <h5>OR</h5>
              {files[docType] && (
                <p className="fileName">{files[docType]?.name}</p>
              )}
              <input
                type="file"
                name={docType}
              ...
              {matchingDoc ? (
                <label htmlFor={docType} style={{ background: "2F4F9E" }}>
                  Change
                </label>
              ) : (
