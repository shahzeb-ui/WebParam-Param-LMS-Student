...
import {
  ...
  getStudentDocuments,
} from "@/app/api/studentProfile/studentprofile";
...
const FileUpload: React.FC = () => {
  ...
  const handleUpload = async (e: any) => {
    ...
    if (selectedFile && selectedFile.file) {
      ...
      if (courseId == "66aa8cab45223bcb337a9643") {
        type = yesProgramme.findIndex(
          (doc) => doc.documentName === selectedFile.type
        );
      } else {
        type = documentsRequired.findIndex(
          (doc) => doc.documentName === selectedFile.type
        );
      }
      ...
      try {
        const response = await axios.post(
          `${writeUserData}/api/v1/Profile/SubmitDocument`,
          formData,
          {
            headers: {
        ...
        if (response.status === 200) {
          setIsUploaded(true);
          router.push(
            `/student/student-profile?tab=documents&document=${selectedDocument}&action=view`
          );
    ...
    if (selectedFile && selectedFile.file) {
      ...
      try {
        const response = await axios.put(
          `${writeUserData}/api/v1/Documents/UpdateStudentDocument`,
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
            ...
            <div>
              ...
              ${
                documentinfo?.status === "Accepted"
                  ? "bg-success"
                  : documentinfo?.status === "PendingReview"
                  ? "bg-warning"
              ...
              <button
                ...
                onClick={() => (
                  router.push(
                    `/student/student-profile?tab=documents&document=${selectedDocument}&action=upload`
                  ),
                  setIsChangingDoc(!isChangingDoc)
                )}
          ...
          <ul className="nav nav-segment nav-pills" role="tablist">
            {courseId == "66aa8cab45223bcb337a9643"
              ? yesProgramme
                  .filter((doc) =>
                    isFreemium
                      ? freemiumDocuments.includes(doc.documentName)
                      : true
                  )
                  .map((doc, index) => {
                    ...
                    return (
                      <li
                        className={`nav-item ${
                          selectedDocument === doc?.documentName ? "active" : ""
                        }`}
                        role="presentation"
                        key={index}
                        onClick={() => (
                          router.push(
                            `/student/student-profile?tab=documents&document=${doc.documentName}&action=view`
                          ),
                          viewDocument(matchingDoc.id)
                        )}
                      >
                        <div className="icon">
                ...
                {documentinfo?.blobUrl ? (
                  <div className="upload-container">
                    ...
                    <form onSubmit={handleChangeDocument}>
                      <div
                        onDrop={(e) =>
                          handleDrop(e, documentinfo as DocumentType)
                        }
                        onDragOver={(e) =>
                          handleDragOver(e, documentinfo as DocumentType)
                        }
                        onDragLeave={(e) =>
                          handleDragLeave(e, documentinfo as DocumentType)
                        }
                        style={{
                          transition: "all 0.5s ease",
                          backgroundColor: dragging[
                            documentinfo as DocumentType
                          ]
                            ? "whitesmoke"
                            : "transparent",
                          border: dragging[documentinfo as DocumentType]
                            ? "2px dashed grey"
                            : "1px dashed black",
                          boxShadow: dragging[documentinfo as DocumentType]
                      ...
                      >
                        ...
                        <input
                          ...
                          name="file-input"
                          onChange={(e) =>
                            handleFileChange(e, documentinfo as DocumentType)
                          }
                          accept=".pdf,.doc,.docx"
                          style={{ display: "none" }}
                        />
                        <label className="btn btn-dark" htmlFor="file-input">
                          Choose File
                        </label>
                      </div>
                      <div>
