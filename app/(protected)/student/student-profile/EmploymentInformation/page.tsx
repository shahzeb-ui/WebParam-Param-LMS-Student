...
import { rUserUrl } from "@/app/lib/endpoints";
          ...
          >
            <div className="rbt-form-group">
              ...
              >
                <option value="">select</option>
                {codes &&
                  codes[18]?.codes?.map((item: any, index: number) => (
                    <option
                      key={index}
                      value={`${item.code}`}
                      className="text-dark"
          ...
          >
            <div className="rbt-form-group">
              ...
              >
                <option value="">Select</option>
                {Array.from(
                  new Set(preferredOccupations.map((item) => item.sector))
                )
                  .filter(
                    (sector) =>
                      sector === selectedSector || selectedSector === ""
                  )
                  .map((sector, index) => (
